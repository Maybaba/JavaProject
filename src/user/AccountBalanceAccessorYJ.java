...
public class AccountBalanceAccessorYJ {
    ...
    private AccountBalanceAccessorYJ() {
        ...
        // 기존에 있던 데이터를 복사
        for (User user : accountBalances.keySet()) {
            Map<AccountType, Long> oldBalances = accountBalances.get(user);
            Map<AccountType, Long> newBalances = new HashMap<>(oldBalances);
            accountBalances.put(user, newBalances);
        }
    }
    ...
    // 클래스 map 계좌 잔액 조회 -> 항상 0을 반환하는듯 함
